...
# for each line, find a two-digit number
# move left pointer (i) until you hit a digit
# save that digit as left_digit
# same for right pointer and right digit
# concatenate left and right digits
    # ? can you concatenate in py? 
    # ? might have to multiply left digit by 10 and add
...
for line in calibration_document:
    ...
    while left_digit == None or right_digit == None:
        if line[i] in digits and left_digit == None:
            left_digit = line[i]

        if line[j] in digits and right_digit == None:
            right_digit = line[j]
    ...
    calibration_value = left_digit + right_digit
